  ...
  return (
    <div>
      <div className="py-5 flex justify-between items-center">
        ...
        {/* Add Product Button  */}
        <Link to={"/addproduct"}>
          <button className="px-5 py-2 bg-pink-50 border hover:bg-pink-200 border-pink-500 rounded-lg">
            Add Product
          </button>
        </Link>
      </div>
        ...
        <table className="w-full text-left border border-collapse sm:border-separate border-pink-100 text-pink-400">
          <tbody style={{ textAlign: 'center' }}>
            ...
            {getAllProduct.map((item, index) => {
              const { id, title, price, category, productSize, date, productImageUrl } =
                item;
              return (
